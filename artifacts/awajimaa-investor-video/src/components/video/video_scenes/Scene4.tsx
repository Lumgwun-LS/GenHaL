  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 w-full h-full flex flex-col items-center justify-center text-center px-12">
        ...
        <motion.p
          className="text-3xl text-gradient-gold font-medium mb-10 max-w-3xl"
          initial={{ opacity: 0, y: 30 }}
          animate={{ opacity: 1, y: 0 }}
        ...
        <div className="flex gap-8 flex-wrap justify-center max-w-4xl mb-10">
          {[
            { title: "B2B Networking", desc: "Digital Business Book" },
            { title: "Multi-Vendor Logistics", desc: "Taxi & freight, all in one" },
            { title: "Insurance Marketplace", desc: "Digital protection at scale" }
          ].map((item, i) => (
            <motion.div
              key={item.title}
        ...
        <motion.div
          className="flex items-center gap-3 px-5 py-3 rounded-full border border-accent/40 bg-accent/10 w-fit"
          initial={{ opacity: 0, y: 10 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ delay: 2.2, duration: 0.8 }}
        >
          <div className="w-2 h-2 rounded-full bg-accent animate-pulse" />
          <span className="text-accent font-mono text-sm tracking-wider">awahubapp.com</span>
        </motion.div>
      </div>
    </motion.div>
  );

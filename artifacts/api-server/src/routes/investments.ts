...
router.get("/investments/export", async (req, res): Promise<void> => {
  ...
  // Stream investments in fixed-size batches ordered by a stable, indexed
  // key (id) so memory usage stays constant regardless of history size —
  // mirrors the admin vendor export's batching pattern.
  ...
  while (true) {
    const batch = await db
      ...
      .where(and(...exportConditions, gt(investmentsTable.id, lastId)))
      .orderBy(asc(investmentsTable.id))
      .limit(BATCH_SIZE);
    if (batch.length === 0) break;

    let chunk = "";
    for (const i of batch) {
      chunk += [i.id, i.type, i.name, i.amount, i.currentValue, i.currency, i.status, i.investmentDate.toISOString()].map(csvCell).join(",") + "\r\n";
    }
    res.write(chunk);
...
const VALID_TYPES = ["owner_capital", "loan", "equity", "external_asset"];

router.post("/investments/import", _csvUpload.single("file"), async (req: any, res: any): Promise<void> => {
  ...
  for (let i = 1; i < rows.length; i++) {
    ...
    const type = VALID_TYPES.includes(rawType) ? rawType : "owner_capital";
    const rawDate = r[col("date")] || r[col("investment_date")] || "";
    const cvRaw = r[col("current_value")] || r[col("currentvalue")] || "";
    try {
      await db.insert(investmentsTable).values({
        ...
        amount: amount.toString(),
        currentValue: cvRaw ? parseFloat(cvRaw).toString() : null,
        status: r[col("status")] === "closed" ? "closed" : "active",
        investmentDate: rawDate ? new Date(rawDate) : new Date(),
      });
      imported++;

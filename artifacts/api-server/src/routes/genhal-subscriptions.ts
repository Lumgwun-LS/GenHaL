...
// ── Get plans (public) ────────────────────────────────────────────────────────
router.get("/genhal/plans", async (req, res) => {
...
router.get("/genhal/subscriptions/:unitType/:unitId", requireAuth(), async (req, res) => {
  try {
    const { unitType, unitId } = req.params;
    const [sub] = await db.select().from(genhalSubscriptionsTable)
      .where(and(eq(genhalSubscriptionsTable.unitType, unitType), eq(genhalSubscriptionsTable.unitId, Number(unitId))));
    if (!sub) {
      // Return a synthetic free plan if no subscription row yet
      return res.json({ plan: "free", status: "active", ...GENHAL_PLANS.free });
    }
    res.json(sub);
...
router.post("/genhal/subscriptions/ensure-free", requireAuth(), async (req, res) => {
...
// ── Stripe checkout ───────────────────────────────────────────────────────────
router.post("/genhal/subscriptions/checkout/stripe", requireAuth(), async (req, res) => {
  const userId = req.auth!.userId;
  const { unitType, unitId, plan, successUrl, cancelUrl } = req.body;
  if (!unitType || !unitId || !plan) return res.status(400).json({ error: "unitType, unitId, plan required" });

  const planDef = GENHAL_PLANS[plan as keyof typeof GENHAL_PLANS];
  if (!planDef || plan === "free") return res.status(400).json({ error: "Invalid plan" });
...
// ── Paystack checkout ─────────────────────────────────────────────────────────
router.post("/genhal/subscriptions/checkout/paystack", requireAuth(), async (req, res) => {
  const userId = req.auth!.userId;
  const { unitType, unitId, plan, email } = req.body;
  if (!unitType || !unitId || !plan || !email) return res.status(400).json({ error: "unitType, unitId, plan, email required" });
...
router.post("/genhal/subscriptions/admin-upgrade", requireAuth(), async (req, res) => {

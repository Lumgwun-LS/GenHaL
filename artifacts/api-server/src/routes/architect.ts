...
/** Wraps async route handlers so any thrown error is forwarded to Express's
 *  error handler rather than becoming an unhandled promise rejection. */
...
async function resolveVendorAccess(req: Request, vendorId: number): Promise<{ ok: true } | { ok: false; status: number; error: string }> {
...
router.get("/architect/projects", asyncHandler(async (req, res) => {
...
router.post("/architect/projects", asyncHandler(async (req, res) => {
...
router.patch("/architect/projects/:id", asyncHandler(async (req, res) => {
...
router.delete("/architect/projects/:id", asyncHandler(async (req, res) => {
  const id = parseInt(req.params.id as string);
  const [existing] = await db.select().from(architectProjectsTable).where(eq(architectProjectsTable.id, id));
  if (!existing) { res.status(404).json({ error: "Project not found" }); return; }
  const check = await resolveVendorAccess(req, existing.vendorId);
...
router.get("/architect/milestones", asyncHandler(async (req, res) => {
...
router.post("/architect/milestones", asyncHandler(async (req, res) => {
...
router.patch("/architect/milestones/:id", asyncHandler(async (req, res) => {
  const id = parseInt(req.params.id as string);
...
router.delete("/architect/milestones/:id", asyncHandler(async (req, res) => {
  const id = parseInt(req.params.id as string);
  const [existing] = await db.select().from(projectMilestonesTable).where(eq(projectMilestonesTable.id, id));
  if (!existing) { res.status(404).json({ error: "Milestone not found" }); return; }
  const check = await resolveVendorAccess(req, existing.vendorId);
...
router.get("/architect/drawings", asyncHandler(async (req, res) => {
  const { vendorId, projectId } = req.query as Record<string, string>;
  if (!vendorId) { res.status(400).json({ error: "vendorId required" }); return; }
  const check = await resolveVendorAccess(req, parseInt(vendorId));
  if (!check.ok) { res.status(check.status).json({ error: check.error }); return; }
...
router.post("/architect/drawings", asyncHandler(async (req, res) => {
...
router.patch("/architect/drawings/:id", asyncHandler(async (req, res) => {
  const id = parseInt(req.params.id as string);
...
router.delete("/architect/drawings/:id", asyncHandler(async (req, res) => {
  const id = parseInt(req.params.id as string);
  const [existing] = await db.select().from(drawingRevisionsTable).where(eq(drawingRevisionsTable.id, id));
  if (!existing) { res.status(404).json({ error: "Drawing not found" }); return; }
  const check = await resolveVendorAccess(req, existing.vendorId);
...
router.get("/architect/contractors", asyncHandler(async (req, res) => {
  const { vendorId, projectId } = req.query as Record<string, string>;
  if (!vendorId) { res.status(400).json({ error: "vendorId required" }); return; }
  const check = await resolveVendorAccess(req, parseInt(vendorId));
  if (!check.ok) { res.status(check.status).json({ error: check.error }); return; }
...
router.post("/architect/contractors", asyncHandler(async (req, res) => {
...
router.patch("/architect/contractors/:id", asyncHandler(async (req, res) => {
  const id = parseInt(req.params.id as string);
...
router.delete("/architect/contractors/:id", asyncHandler(async (req, res) => {
  const id = parseInt(req.params.id as string);
  const [existing] = await db.select().from(contractorTasksTable).where(eq(contractorTasksTable.id, id));
  if (!existing) { res.status(404).json({ error: "Task not found" }); return; }
  const check = await resolveVendorAccess(req, existing.vendorId);
...
router.get("/architect/floor-plans", asyncHandler(async (req, res) => {
  const { vendorId, projectId } = req.query as Record<string, string>;
  if (!vendorId) { res.status(400).json({ error: "vendorId required" }); return; }
  const check = await resolveVendorAccess(req, parseInt(vendorId));
  if (!check.ok) { res.status(check.status).json({ error: check.error }); return; }
...
router.post("/architect/floor-plans", asyncHandler(async (req, res) => {
...
router.patch("/architect/floor-plans/:id", asyncHandler(async (req, res) => {
  const id = parseInt(req.params.id as string);
...
router.delete("/architect/floor-plans/:id", asyncHandler(async (req, res) => {
  const id = parseInt(req.params.id as string);
  const [existing] = await db.select().from(floorPlansTable).where(eq(floorPlansTable.id, id));
  if (!existing) { res.status(404).json({ error: "Floor plan not found" }); return; }
  const check = await resolveVendorAccess(req, existing.vendorId);
...
router.get("/architect/designs", asyncHandler(async (req, res) => {
...
router.post("/architect/generate-design", asyncHandler(async (req, res) => {
  ...
  try {
    ...
    // Surface a friendly error — content policy, rate limit, etc.
...
router.post("/architect/edit-design", upload.single("image"), asyncHandler(async (req, res) => {
  ...
  let dallePrompt = editPrompt.trim();
  try {
...
Return ONLY the DALL-E 3 prompt — no explanation, no preamble.`,
    ...
    dallePrompt = visionRes.choices[0]?.message?.content?.trim() ?? editPrompt.trim();
  } catch {
    // Vision analysis failed — fall back to using the edit prompt directly
  }
  ...
  try {
    ...
    imageUrl = imgResponse.data?.[0]?.url ?? null;
    revisedPrompt = imgResponse.data?.[0]?.revised_prompt ?? null;
  } catch (err: unknown) {
    const msg = err instanceof Error ? err.message : String(err);
    res.status(502).json({ error: `Image generation failed: ${msg}` });
    return;
...
router.get("/architect/designs/:id/image", asyncHandler(async (req, res) => {
  const id = parseInt(req.params.id as string);
